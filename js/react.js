...
(function () {
  ...
  /**
   * Map numeric risk_score → visual tier string.
   * Used to pick ambient glow color and score ring color.
   */
  ...
  /** Human-readable verdict from score */
      ...
      const rows = Object.entries(check.data)
        .map(([k, v]) => `
          <div class="check-data-row">
            ...
            <span class="check-data-val">${UI.formatValue(v)}</span>
          </div>`)
        .join("");
  ...
  function render(data) {
    ...
    const verdict = UI.formatValue(data.verdict);
